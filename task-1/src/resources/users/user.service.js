...
const get = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ where: { firstName: name } }).then(
      (user) => {
        return user
      }
    )
    ...
    let filename = `./static/assets/uploads/${user.firstName}_${
      user.lastName
    }_${Date.now()}.pdf`
...
const create = (req, res) => {
  ...
  try {
    ...
    User.create({
      firstName: firstname,
      lastName: lastname,
      image: fs.readFileSync('./static/assets/uploads/' + req.filename)
    }).then(() => {
